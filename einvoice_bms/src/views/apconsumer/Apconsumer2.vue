<template>
    <div>
        <h1 style="text-align: center">載具消費發票查詢</h1>
    </div>
    <div class="tdiv">
        <p>
    ...
    <div class="cbox">
        ...
        <div>
            ...
            <input class="rdiv" type="date" v-model="sdate">
            <input class="rdiv" type="date" v-model="edate">
        </div>
        <div>
            <label class="col-sm-3 control-label" style="text-align:right">
                <span class="redstar">*</span>
            ...
            <select class="rdiv" v-model="invoices">
                <option>全部發票</option>
                <option>未捐贈發票</option>
                <option>已捐贈發票</option>
                <option>已作廢發票</option>
    ...
    <div class="sbox">
        ...
        <div style="margin-left:20px">
            1.依統一發票使用辦法第7條規定，營業人應於發票開立後48小時內將發票資訊傳輸至本平台，方能查詢。<br>
            2.單月25日起，不可再捐贈當月以前的發票，例：7/25起，不可捐贈7/1以前的發票。 <br>
            3.會員載具請至營業人會員平台查詢
        </div>
    </div>
</template>
...
<script setup>
    ...
    function checkValue() {
        ...
        if (apoption.value === '手機條碼') {
            ...
            if (checkNum.test(phonen.value)) {
                if (testn.value !== undefined && testn.value !== '') {
                    if (sdate.value !== undefined && edate.value !== undefined && edate.value >= sdate.value) {
        ...
        if (apoption.value === '自然人憑證條碼' && cnaturer.value === '輸入自然人憑證卡號和自然人憑證條碼密碼') {
            console.log("自然人憑證條碼+輸入自然人憑證卡號和自然人憑證條碼密碼")
        ...
        if (apoption.value === '一卡通' && ipass.value === '輸入卡片內碼和卡片驗證碼') {
            ...
            if (ipasscard.value !== undefined && ipasscard.value !== '') {
                if (ipassvalid.value !== undefined && ipassvalid.value !== '') {
                    if (sdate.value !== undefined && edate.value !== undefined && edate.value >= sdate.value) {
                        if (pic.value == data.img_code) {
                            router.push({
                                path: '/apresult/2',
    ...
    onMounted(() => {
        console.log(sdate.value === undefined)
